...
export const MessageList = ({ conversationId }) => {
  ...
  const renderMessage = (message) => {
    ...
    return (
      ...
      >
        <div className={`flex ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'} items-end gap-2`}>
          <img
          ...
          <div className={`max-w-[70%] ${isOwnMessage ? 'text-right' : 'text-left'}`}>
            {message.type === 'image' ? (
              <img
                src={message.content}
                alt="消息图片"
                className="max-w-full rounded-2xl cursor-pointer hover:opacity-90 transition-opacity border border-gray-200 dark:border-gray-700"
                onClick={() => window.open(message.content, '_blank')}
              />
            ) : (
              <div
                className={`px-4 py-2 rounded-2xl shadow-sm text-[15px] leading-tight break-words whitespace-pre-wrap transition-all ${
                  isOwnMessage
                    ? 'bg-twitter-blue text-white rounded-br-md'
                    : 'bg-gray-100 dark:bg-[#232d36] text-gray-900 dark:text-white rounded-bl-md'
                }`}
              >
                {message.content}
              </div>
